...
for(st in c(1:27)[-c(6,7,11,23,27)]){
  ...
  pessoa <- read.csv(paste0("Pessoa03_",estado,".csv"), header = TRUE, sep=";",encoding = "UTF-8")
  dom.renda <- read.csv(paste0("DomicilioRenda_",estado,".csv"), header = TRUE, sep=";",encoding = "UTF-8")
  resp.alfa <- read.csv(paste0("Responsavel02_",estado,".csv"), header = TRUE, sep=";",encoding = "UTF-8")
...
#save(ipc.full,file = "C:/Users/pchir/Google Drive/AcaoCovid/indice_vulnerabilidade-master/Pacote_IPC/EspirituSanto/IPC_FULL_UF_MUNI_BAIRRO_ES.rda")

#install.packages("snakecase")
#snakecase::to_title_case()

ipc.full%>%str
#write.csv2(ipc.Muni.ES,file = "IPC_ES.csv",row.names = FALSE)

IPC.ES.Bairro <- ipc.full%>%dplyr::select(-c("compEntorno","compDomRenda","compDomiciliosMulher",
                          "comp5maisdomicilio","compbanheiro","compagua","compPessoas","Nome_UF"))

#write.csv2(IPC.ES.Bairro,file = "IPC_ES_Bairro.csv",row.names = FALSE)

library("readxl")

setwd("C:/Users/pchir/Google Drive/AcaoCovid/indice_vulnerabilidade-master/Pacote_IPC/EspirituSanto")

ipc.Muni.ES <- ipc.full%>%group_by(Nome_do_municipio)%>%summarise_at("ipc",mean)%>%
  `colnames<-`(c("NOME","IPC"))

# save(ipc.Muni.ES,file = "IPC_ES.rda")

ipc.Muni.ES <- readRDS("IPC_ES.rds", refhook = NULL)
...
ipc.Muni.ES$NOME <- firstup(ipc.Muni.ES$NOME)

ipc.Muni.ES%>%rename(name=NOME)

#write_excel_csv2(ipc.Muni.ES,"IPC_ES.csv")
write.csv2(ipc.Muni.ES,file = "IPC_ES.csv",row.names = FALSE)
